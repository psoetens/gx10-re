"""
WinMM-based MIDI sniffer. Opens a MIDI INPUT port by name (default 'GX-10') and
logs every short message and SysEx with a high-resolution timestamp + direction
tag, both to stdout (one event per line) and to a JSONL log file.

Direction tagging: WinMM only lets us snoop the input direction (device -> host).
Output direction (host -> device) is captured separately by also opening the
GX-10 OUTPUT port if it's free — most drivers won't let two openers coexist on
output, so when --mitm is requested we run a small bidirectional proxy instead.
For pure passive observation we just sniff input.
...
# Set argtypes/restype on every winmm function we call so 64-bit pointers
# don't get silently truncated to 32-bit ints by ctypes' default c_int.
...
class MIDIHDR(ctypes.Structure):
    # IMPORTANT: lpData must be c_void_p (raw pointer) and NOT c_char_p,
    # because c_char_p auto-marshals to a NUL-terminated Python bytes object
    # whenever you read it back, which would silently truncate / corrupt
    # the SysEx data we're trying to capture.
    _fields_ = [
...
class Sniffer:
    ...
    def _callback(self, hMidiIn, wMsg, dwInstance, dwParam1, dwParam2):
        try:
            if wMsg == MIM_DATA:
                # dwParam1 is a DWORD packed as: status | data1<<8 | data2<<16
                ...
                # dwParam1 is a pointer to MIDIHDR
                hdr_ptr = dwParam1
    ...
    def open(self):
        ...
        # Allocate SysEx buffers and add them
...
def main():
    ...
    # Optional label fifo: poll the file for new lines and tag the stream.
